...
独立的项目设计文档，适用对象： **PC game performance**。
...
- **帧时间 CSV 导入** (Frame-time CSV import) — 规划模块，尚未实现。
- **百分位比较表** (Percentile comparison table) — 规划模块，尚未实现。
- **基准测试条件清单** (Benchmark condition manifest) — 规划模块，尚未实现。
